...
###1114
from diffusers_edit import QwenImageEditPipeline
...
from flow_grpo.diffusers_patch.qwenimage_edit_pipeline_with_logprob_t2i import pipeline_with_logprob
...
class GenevalPromptDataset(Dataset):
    def __init__(self, dataset, split='train'):
        ...
        self.file_path = os.path.join(dataset, f'{split}_metadata_1.jsonl')
...
def compute_log_prob(transformer, pipeline, sample, j, config, rank):
    ...
    calculated_width, calculated_height, _ = calculate_dimensions(config.resolution * config.resolution, 1)
    # img_shapes = [
    #     [
    ...
    # if sample["image_latents"] is not None and None not in sample["image_latents"]:
...
def eval(pipeline, test_dataloader, config, rank, local_rank, world_size, device, global_step, reward_fn, executor, autocast, ema, transformer_trainable_parameters):
    ...
    for test_batch in tqdm(
        ...
        with autocast():
            with torch.no_grad():
                collected_data = pipeline_with_logprob(
                        ...
                        ##1115
                        true_cfg_scale=config.sample.guidance_scale, #1,#
                        output_type="pt",
        ...
        ##1112
        #images = torch.tensor([1 if x is None else x for x in collected_data["image_latents"]]) #collected_data["images"]
